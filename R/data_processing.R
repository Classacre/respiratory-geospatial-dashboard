...
generate_synthetic_data <- function(n_patients = 5000, seed = 42) {
  ...
  # NEW: Medication use indicator (controller vs rescue medication)
  # Controller medications: ICS, LABA/ICS combination
  # Rescue medications: SABA (short-acting beta agonist)
  controller_use <- ifelse(asthma_diagnosis == 1,
                           rbinom(n_patients, 1, 0.6),  # 60% of asthmatics on controllers
                           0)
  rescue_use <- ifelse(asthma_diagnosis == 1,
                       rbinom(n_patients, 1, 0.85),  # 85% of asthmatics have rescue inhaler
                       rbinom(n_patients, 1, 0.1))   # 10% of non-asthmatics have rescue inhaler

  medication_use <- ifelse(controller_use == 1, "Controller",
                           ifelse(rescue_use == 1, "Rescue Only", "None"))
  medication_use <- factor(medication_use, levels = c("None", "Rescue Only", "Controller"))

  # NEW: Quality of care index (composite of outpatient visits and medication adherence)
  # Number of outpatient visits (higher for well-controlled asthma)
  outpatient_visits <- rpois(n_patients, lambda = ifelse(asthma_diagnosis == 1, 3, 1))

  # Medication adherence score (0-100, higher is better)
  medication_adherence <- ifelse(asthma_diagnosis == 1,
                                  pmin(pmax(rnorm(n_patients, 65, 20), 0), 100),
                                  NA)

  # Quality of care index (0-100 scale)
  quality_care_index <- ifelse(asthma_diagnosis == 1,
                               (outpatient_visits / 5 * 40) +  # 40% weight on visits
                                 (medication_adherence * 0.6),  # 60% weight on adherence
                               NA)
  quality_care_index <- pmin(pmax(quality_care_index, 0), 100)

  # NEW: Healthcare utilization score
  # ED visits
  ed_visits <- rpois(n_patients, lambda = ifelse(asthma_diagnosis == 1,
                                                   ifelse(exacerbation_count > 3, 1.5, 0.3),
                                                   0.05))

  # Calculate healthcare utilization score
  # Higher score indicates more intensive healthcare use
  # Formula: (ED visits * 3 + hospitalizations * 5 - outpatient visits * 0.5)
  # Normalized to 0-100 scale
  raw_utilization <- (ed_visits * 3) + (hospitalization_count * 5) - (outpatient_visits * 0.5)
  healthcare_utilization <- (raw_utilization - min(raw_utilization)) /
    (max(raw_utilization) - min(raw_utilization)) * 100
  ...
  baseline_data <- data.frame(
    ...
    controller_use = factor(controller_use, labels = c("No", "Yes")),
    rescue_use = factor(rescue_use, labels = c("No", "Yes")),
    outpatient_visits = outpatient_visits,
    medication_adherence = round(medication_adherence, 1),
    quality_care_index = round(quality_care_index, 1),
    ed_visits = ed_visits,
    healthcare_utilization = round(healthcare_utilization, 1),
  ...
  for (i in seq_len(nrow(baseline_data))) {
    ...
    for (v in 1:n_visits) {
      ...
      if (v > 1) {
        ...
        # Update healthcare utilization with new exacerbations
        new_ed <- rpois(1, lambda = new_exacerbations * 0.3)
        visit_data$ed_visits <- patient$ed_visits + new_ed

        # Recalculate healthcare utilization
        raw_util <- (visit_data$ed_visits * 3) +
          (visit_data$hospitalization_count * 5) -
          (visit_data$outpatient_visits * 0.5)
        visit_data$healthcare_utilization <- round(
          (raw_util - min(baseline_data$ed_visits * 3)) /
            (max(baseline_data$ed_visits * 3) - min(baseline_data$ed_visits * 3)) * 100, 1
        )
      }
